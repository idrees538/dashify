...
function ProjectFolder({ project, isActive, onClick }) {
    return (
        <button
            className={`group w-full bg-bg-secondary rounded-lg shadow-sm border overflow-hidden cursor-pointer transition-all duration-200 text-left hover:shadow-lg ${isActive ? 'border-accent shadow-[0_0_0_2px_var(--accent-light)]' : 'border-border-color'}`}
            onClick={onClick}
        >
            <div className={`relative w-full h-40 flex items-center justify-center
                ...
                <div className="w-14 h-14 rounded-lg bg-white/12 backdrop-blur-md flex items-center justify-center text-2xl text-white transition-all duration-200 border border-white/15 group-hover:scale-110">
                    <IoFolderOutline />
                </div>
            ...
            <div className="p-3.5 px-4">
                <h3 className="text-sm font-semibold text-text-primary mb-0.5">{project.name}</h3>
                <p className="text-[11px] text-text-secondary">{project.client} · {project.date}</p>
            </div>
        </button>
    );
}
