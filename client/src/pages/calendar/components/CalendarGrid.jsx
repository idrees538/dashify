...
export default function CalendarGrid({
    ...
    return (
        <div className="flex-1 flex flex-col min-h-0 border border-[var(--border-color)] rounded-xl overflow-hidden bg-[var(--bg-card)]">
            {/* Day-of-week headers */}
            <div className="grid grid-cols-7 bg-[var(--bg-primary)]">
                {DAY_NAMES.map((d) => (
                    <div
                        key={d}
                        className="py-2.5 text-center text-xs font-semibold uppercase tracking-wider
                       text-[var(--text-secondary)] border-r border-b border-[var(--border-color)]
                       last:border-r-0"
                    >
                        {d}
                    </div>
